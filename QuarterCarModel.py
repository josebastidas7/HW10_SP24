...
class CarModel():
    ...
    def __init__(self):
        ...
        self.m1 = 450  # $JES MISSING CODE# # mass of car body in kg; this is mass of the quarter of a car
        self.m2 = 25  # $JES MISSING CODE#  # mass of wheel in kg; this is an estimated value
        self.c1 = 2000  # $JES MISSING CODE#  # damping coefficient in N*s/m
        self.k1 = 50000  # $JES MISSING CODE#  # spring constant of suspension in N/m
        self.k2 = 200000  # $JES MISSING CODE#  # spring constant of tire in N/m
        self.v = 60  # $JES MISSING CODE#  # velocity of car in kph
        # Note I took a rough estimate to  the min and maxes and even spread them out further once I saw my calculations hit the limit when optimizing
        self.mink1 = 15000  # $JES MISSING CODE#  #If I jack up my car and release the load on the spring, it extends about 3 inches
        self.maxk1 = 70000  # $JES MISSING CODE#  #What would be a good value for a soft spring vs. a stiff spring?
        self.mink2 = 100000  # $JES MISSING CODE#  #Same question for the shock absorber.
        self.maxk2 = 300000  # $JES MISSING CODE#
        self.accel = None
        self.accelMax = 2 * 9.81  # $JES MISSING CODE#
        self.accelLim = 2 * 9.81  # $JES MISSING CODE#
        self.SSE = 0.0
...
class CarController():
    ...
    def ode_system(self, X, t):
        ...
        x1ddot = ((self.model.k1 * (x2 - x1)) + (
                    self.model.c1 * (x2dot - x1dot))) / self.model.m1  # $JES MISSING CODE#)
        x2ddot = ((self.model.k2 * (y - x2)) - (self.model.k1 * (x2 - x1)) - (
                    self.model.c1 * (x2dot - x1dot))) / self.model.m2  # $JES MISSING CODE#
    ...
    def calculate(self, doCalc=True):
        ...
        self.model.m1 = float(self.le_m1.text())  # $JES MISSING CODE#
        self.model.m2 = float(self.le_m2.text())  # $JES MISSING CODE#
        self.model.c1 = float(self.le_c1.text())  # $JES MISSING CODE#
        self.model.k1 = float(self.le_k1.text())  # $JES MISSING CODE#
        self.model.k2 = float(self.le_k2.text())  # $JES MISSING CODE#
        self.model.v = float(self.le_v.text())  # $JES MISSING CODE#

        # recalculate min and max k values
        self.mink1 = self.model.m1 * 9.81 / 0.1524  # $JES MISSING CODE#
        self.maxk1 = self.model.m1 * 9.81 / 0.0762  # $JES MISSING CODE#
        self.mink2 = (self.model.m2 + self.model.m1) * 9.81 / 0.0381  # $JES MISSING CODE#
        self.maxk2 = (self.model.m2 + self.model.m1) * 9.81 / 0.01905  # $JES MISSING CODE#
    ...
    def OptimizeSuspension(self):
        ...
        # Step 2:
        # JES MISSING CODE HERE$
        x0 = np.array([self.model.k1, self.model.c1,
                       self.model.k2])  # create a numpy array with initial values for k1, c1, and k2
        # Step 3:
        # JES MISSING CODE HERE$
        answer = minimize(self.SSE, x0, method='nelder-mead', options={'xatol': 1e-8,'disp': True})
        # use the Nelder-Mead method to minimize the SSE function (our objective function)
